  ...
  return (
    <main className="w-full bg-white">
      ...
      {/* =========================
          DEVICE INSTRUCTIONS
      ========================== */}
      <section className="relative overflow-hidden bg-white px-4 pb-24 pt-12 sm:px-6">

        {/* Device Grid */}
        <div className="relative z-10 mx-auto grid max-w-142.5 grid-cols-2 gap-3 sm:grid-cols-3 mb-30">

          {instructions.map((item, index) => (
            <NavLink
              to={item.path}
              key={`${item.title}-${index}`}
              className="group flex h-36.25 flex-col items-center justify-center rounded-md bg-[#f5f5f5] p-3 transition duration-300 hover:-translate-y-1 hover:shadow-md"
            >
              {/* Device Image */}
              <div className="flex h-22.5 w-full items-center justify-center">
                <img
                  src={item.image}
                  alt={item.title}
              ...
              {/* Device Title */}
              <h2 className="mt-2 text-center text-sm font-medium text-gray-900 transition-colors group-hover:text-blue-700">
                {item.title}
              </h2>
            </NavLink>
          ))}
        ...
        {/* =========================
            BOTTOM WAVE SVG
        ========================== */}
       <div className="pointer-events-none absolute bottom-0 left-0 w-full flex flex-col gap-5">
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 283.5 27.8"
    preserveAspectRatio="none"
    className="block h-16 w-full transform-[scaleY(-1)]"
  >
    <path
      fill="#f5f7f8"
